...
F9 shows that the student's gradient *magnitude* tracks the teacher's; F10 shows that the *direction* does too. The character teacher's `nabla Re T = -(2 pi / p) * sin(2 pi (x + y) / p) * (1, 1)` is rigidly parallel to the `(1, 1)` diagonal at every point on the torus, with scalar length oscillating sinusoidally along the anti-diagonal. The student inherits this direction lock under `H^2` distillation:

![F10: gradient direction overlay at p = 17](figures/f10_grad_quiver_p17.png)

*F10 (Re channel, p = 17, canonical 32x64 baseline_siren): blue streamlines trace the analytic teacher gradient field `nabla Re T`, which is uniformly along the `(1, 1)` diagonal everywhere on the torus, so the streamlines come out as a perfectly parallel diagonal raster. Red arrows are the student's autodiff gradient `nabla Re f_theta` sampled on an 8 x 8 subgrid. The student arrows lie along the streamlines almost everywhere -- direction lock is inherited from the teacher under `H^2` distillation. Direction matching is the qualitative companion to F9's magnitude matching: together they certify that the student tracks the teacher's full first-order differential geometry, not just its magnitude.*
