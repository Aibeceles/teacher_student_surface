...
*F10 (Re channel, p = 17, canonical 32x64 baseline_siren, mesh subsampled to 8 x 8 = 64 arrows per layer): blue arrows are the analytic teacher gradient `nabla Re T`; red arrows are the student's autodiff gradient `nabla Re f_theta` rooted at the same lattice points. The teacher's diagonal-locked pattern is reproduced almost everywhere; the few visible red-blue divergences cluster at the same scattered points where F9's mismatch panel shows brighter spots. Direction matching is the qualitative companion to F9's magnitude matching: together they certify that the student tracks the teacher's full first-order differential geometry, not just its magnitude.*
